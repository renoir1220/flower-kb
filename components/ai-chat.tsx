    ...
    <div className="text-center py-12">
      ...
      <div className="flex flex-wrap justify-center gap-2">
        {["蝴蝶兰怎么养", "帮我查一下绿萝", "添加一个新植物"].map((s) => (
          <button key={s} onClick={() => setInput(s)} className="px-4 py-2 rounded-full bg-secondary/50 hover:bg-secondary text-sm">{s}</button>
        ))}
      </div>
    </div>
  );
        ...
        <div className="max-w-3xl mx-auto px-6 py-8 space-y-6">
          ...
          {isLoading && messages[messages.length - 1]?.role !== "assistant" && (
            <div className="flex gap-3">
              ...
              <div className="bg-secondary/50 rounded-2xl px-4 py-3"><Loader2 className="w-5 h-5 animate-spin" /></div>
            </div>
          )}
        </div>
      </div>
...
export function AIChatWidget({ hideOnRoutes = ["/ai", "/esse"] }: { hideOnRoutes?: string[] }) {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3">
      {isOpen && (
        <div className="w-[360px] h-[520px] rounded-2xl border bg-background shadow-2xl overflow-hidden flex flex-col">
      ...
      <Button size="icon" className="h-12 w-12 rounded-full shadow-lg" onClick={() => setIsOpen((p) => !p)}>
